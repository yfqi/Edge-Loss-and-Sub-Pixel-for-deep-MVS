...
class DepthNet(nn.Module):
    ...
    def forward(self, stage_idx,features, proj_matrices, depth_values1,depth_values, num_depth, cost_regularization, prob_volume_init=None):
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            ...
            warped_volume = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values1)
        ...
        if stage_idx == 0:
            prob_volume_pre = self.subpixel(prob_volume_pre,0)
        if stage_idx == 1:
            prob_volume_pre = self.subpixel(prob_volume_pre,1)
...
class Subpixel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(48, 48 * 16, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv2 = nn.Conv2d(32, 4 * 32, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self,input,stage):
        ...
        if stage == 0:
            x = self.conv1(input)
            y = self.pixel_shuffle1(x)
        if stage == 1:
            x = self.conv2(input)
            y = self.pixel_shuffle2(x)
        return y
...
class CascadeMVSNet(nn.Module):
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for nview_idx in range(imgs.size(1)):  #imgs shape (B, N, C, H, W)
            ...
            #features are all level feature maps of each views

        outputs = {}
        depth, cur_depth = None, None
        for stage_idx in range(self.num_stage):
            #the first is the lowest feature map,then the larger
            ...
            outputs_stage = self.DepthNet(stage_idx,features_stage, proj_matrices_stage,
                                          depth_values=F.interpolate(depth_range_samples.unsqueeze(1),
                                                                      [self.ndepths[stage_idx], img.shape[2], img.shape[3]], mode='trilinear',
                                                                      align_corners=Align_Corners_Range).squeeze(1),
                                          depth_values1=F.interpolate(depth_range_samples.unsqueeze(1),
        ...
        outputs['feats'] = features_stage[0].permute([0, 2, 3, 1]).detach()
