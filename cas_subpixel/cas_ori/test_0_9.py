...
from EfficientMVS.filter import dypcd_filter
...
parser.add_argument('--outdir', default='/media/public/yan2/yfq/exp/ec/cas_pointrend_0.9/out/10_0_gip_tank', help='output dir')
...
#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='/media/public/yan2/yfq/fusibile-master/fusibile')
parser.add_argument('--prob_threshold', type=float, default='0.96')
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            ####################################################################
            tochange = outputs["photometric_confidence"] < 0
            indexs = np.argwhere(outputs["photometric_confidence"] < 0)
            ...
            for index in indexs:
                ...
                if ((index[1] < 10) or (index[2] < 10) or (index[1] > (imgs.shape[3] - 10)) or (
                        index[2] > (imgs.shape[4] - 10)) or (tochange[
                                                                 index[0], index[1], index[2]] == False)):
                    continue

                min_disp = float('inf')

                while (tochange[index[0], index[1], index[2]] == True):

                    disp1 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1] - 1, index[2] - 1]) ** 2).sum()
                    if min_disp > disp1 and ([index[0], index[1] - 1, index[2] - 1] not in list1):
                        min_disp = disp1
                        l = -1
                        r = -1
                    disp2 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1] - 1, index[2]]) ** 2).sum()
                    if min_disp > disp2 and ([index[0], index[1] - 1, index[2]] not in list1):
                        min_disp = disp2
                        l = -1
                        r = 0
                    disp3 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1] - 1, index[2] + 1]) ** 2).sum()
                    if min_disp > disp3 and ([index[0], index[1] - 1, index[2] + 1] not in list1):
                        min_disp = disp3
                        l = -1
                        r = 1
                    disp4 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1], index[2] - 1]) ** 2).sum()
                    if min_disp > disp4 and ([index[0], index[1], index[2] - 1] not in list1):
                        min_disp = disp4
                        l = 0
                        r = -1
                    disp5 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1], index[2] + 1]) ** 2).sum()
                    if min_disp > disp5 and ([index[0], index[1], index[2] + 1] not in list1):
                        min_disp = disp5
                        l = 0
                        r = 1
                    disp6 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1] + 1, index[2] - 1]) ** 2).sum()
                    if min_disp > disp6 and ([index[0], index[1] + 1, index[2] - 1] not in list1):
                        min_disp = disp6
                        l = 1
                        r = -1
                    disp7 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1] + 1, index[2]]) ** 2).sum()
                    if min_disp > disp7 and ([index[0], index[1] + 1, index[2]] not in list1):
                        min_disp = disp1
                        l = 1
                        r = 0
                    disp8 = ((outputs['feats'][index[0], index[1], index[2]] - outputs['feats'][
                        index[0], index[1] + 1, index[2] + 1]) ** 2).sum()
                    if min_disp > disp8 and ([index[0], index[1] + 1, index[2] + 1] not in list1):
                        min_disp = disp8
                        l = 1
                        r = 1
                    if (index[1] < 3 or index[2] < 3 or index[1] > (imgs.shape[3] - 3) or (
                            index[2] > imgs.shape[4] - 3)):

                        tochange[index[0], index[1], index[2]] = False
                        tochange[index[0], index[1] + l, index[2] + r] = False
                        outputs["depth"][index[0], index[1], index[2]] = outputs["depth"][
                            index[0], index[1] + l, index[2] + r]
                        outputs["photometric_confidence"][index[0], index[1], index[2]] = 0.851
                        outputs["photometric_confidence"][index[0], index[1] + l, index[2] + r] = 0.851

                        while len(list1) > 0:
                            elem = list1.pop()
                            tochange[elem[0], elem[1], elem[2]] = False
                            outputs["depth"][elem[0], elem[1], elem[2]] = outputs["depth"][
                                index[0], index[1] + l, index[2] + r]
                            outputs["photometric_confidence"][elem[0], elem[1], elem[2]] = 0.901
                    elif (tochange[index[0], index[1] + l, index[2] + r] == False):
                        tochange[index[0], index[1], index[2]] = False
                        outputs["photometric_confidence"][index[0], index[1], index[2]] = 0.901
                        outputs["depth"][index[0], index[1], index[2]] = outputs["depth"][
                            index[0], index[1] + l, index[2] + r]
                        ...
                        list1.append([index[0], index[1], index[2]])
                        index[1] = index[1] + l
                        index[2] = index[2] + r
...
if __name__ == '__main__':
    ...
    if args.filter_method != "gipuma":
        ...
        dypcd_filter(args, testlist, args.num_worker)
    else:
